...
void setup_blackjack(struct Card* deck_ptr, struct Player* player, struct Dealer* dealer, int* top_deck_count_ptr) {
    ...
    *(player->hand + player->hand_count) = draw;
    player->hand_count++;
    (*top_deck_count_ptr)--;
    ...
    *(player->hand + player->hand_count) = draw;
    ...
    draw = *(deck_ptr + *top_deck_count_ptr);
    *(dealer->hand + dealer->hand_count) = draw; //error
    dealer->hand_count++;
    (*top_deck_count_ptr)--;

    draw = *(deck_ptr + *top_deck_count_ptr);
    *(dealer->hand + dealer->hand_count) = draw;
    dealer->hand_count++;
    (*top_deck_count_ptr)--;
...
int rank_to_int(char rank_str[3]) {
    ...
    if (rank_str[0] == 'A') {
        printf("%s", "There is an Ace in the hand. Would you like to count it as 1 or 11?\n");
        scanf("%d", &ace_value);
        // needs enforcing of ace_value to be 1 of 11.
        return ace_value;
    }
